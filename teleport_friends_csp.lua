...
local function refreshPlayers()
  ...
  -- Car 0 = kendi aracımız.
  -- Direkt ac.getCar() kullanıyoruz; böylece iterateCars() kaynaklı
  -- oyuncu tarama problemlerini azaltıyoruz.
  for i = 1, count - 1 do
...
local function teleportBehind(carIndex, playerName)
  ...
  -- Aynı yöne bakacak şekilde teleport et.
  -- Eski koddaki -look kaldırıldı; bu 180 derece ters dönme
  -- problemini engeller.
...
local teleportApp = ui.addSettings({
  ...
  -- Assetto Corsa/CSP'nin pencere fade sistemini kullan.
  -- Mouse pencerenin uzerinden cekildiginde windowFading 1'e
  -- yaklasir; mouse tekrar gelince 0'a doner.
  local windowFading = ac.windowFading()
  if windowFading > 0.99 then
    return
  end
  ui.pushStyleVarAlpha(1.0 - windowFading)
