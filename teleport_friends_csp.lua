...
local teleportApp = ui.addSettings({
  ...
  if #players == 0 then
    ...
    ui.childWindow(
      ...
      true,
      function()
        -- Listeyi normal durumda çok saydam tut.
        -- Mouse üstüne gelince hover rengi belirginleşir.
        for _, player in ipairs(players) do
          ui.pushID(player.index)

          ui.pushStyleColor(ui.StyleColor.Button, rgbm(1, 1, 1, 0.035))
          ui.pushStyleColor(ui.StyleColor.ButtonHovered, rgbm(1, 1, 1, 0.38))
          ui.pushStyleColor(ui.StyleColor.ButtonActive, rgbm(1, 1, 1, 0.48))
